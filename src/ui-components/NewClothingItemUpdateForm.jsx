...
import { getNewClothingItem } from "../graphql/queries";
import { updateNewClothingItem } from "../graphql/mutations";
export default function NewClothingItemUpdateForm(props) {
  const {
    id: idProp,
    newClothingItem: newClothingItemModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = newClothingItemRecord
      ? { ...initialValues, ...newClothingItemRecord }
      : initialValues;
    setClothingChoice(cleanValues.ClothingChoice);
    setSize(cleanValues.Size);
    setClothingName(cleanValues.ClothingName);
    setClothingBrand(cleanValues.ClothingBrand);
    setImage(cleanValues.image);
    setErrors({});
  };
  const [newClothingItemRecord, setNewClothingItemRecord] = React.useState(
    newClothingItemModelProp
  );
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? (
            await API.graphql({
              query: getNewClothingItem.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getNewClothingItem
        : newClothingItemModelProp;
      setNewClothingItemRecord(record);
    };
    queryData();
  }, [idProp, newClothingItemModelProp]);
  React.useEffect(resetStateValues, [newClothingItemRecord]);
  const validations = {
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await API.graphql({
            query: updateNewClothingItem.replaceAll("__typename", ""),
            variables: {
              input: {
                id: newClothingItemRecord.id,
                ...modelFields,
              },
            },
          });
      ...
      {...getOverrideProps(overrides, "NewClothingItemUpdateForm")}
      {...rest}
    >
      <TextField
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || newClothingItemModelProp)}
